...
def add_cover_page(doc, title_type, title_name, student_name='', student_id=''):
    """添加封面页"""
    for _ in range(3):
        doc.add_paragraph()
...
def create_survey_report():
    ...
    add_body_paragraph(doc, '广东省中山市作为孙中山先生的故乡，不仅有着深厚的历史文化底蕴，更孕育了众多独具特色的地标食品。石岐乳鸽、小榄脆肉鲩、黄圃腊味、沙溪扣肉等美食早已声名远播，成为中山饮食文化的重要名片。为深入了解中山地标食品的传承现状与发展前景，本人于2025年4月至5月期间，对中山市石岐区、小榄镇、黄圃镇等地进行了实地调研，通过走访老字号店铺、食品加工企业、餐饮从业者及消费者，全面了解中山地标食品的历史渊源、制作工艺、市场现状及发展趋势。')
    ...
    add_body_paragraph(doc, '本次调研历时约一个月，分为三个阶段进行：')
    add_body_paragraph(doc, '第一阶段（2025年4月10日-4月20日）：前期准备与资料收集。通过查阅文献资料、网络搜索等方式，系统了解中山地标食品的基本情况，确定调研对象和调研路线，设计访谈提纲和调查问卷。')
    add_body_paragraph(doc, '第二阶段（2025年4月21日-5月5日）：实地调研与访谈。先后走访了石岐区的乳鸽专卖店、小榄镇的脆肉鲩养殖基地和餐饮企业、黄圃镇的腊味生产加工企业、沙溪镇的传统餐饮店铺等，对从业人员、企业管理者、消费者等进行了深入访谈。')
    add_body_paragraph(doc, '第三阶段（2025年5月6日-5月12日）：资料整理与报告撰写。对调研收集的资料进行系统整理和分析，撰写调研报告。')
    
    add_image_placeholder(doc, '图1 走访黄圃腊味传统制作工坊')
    ...
    add_image_placeholder(doc, '图2 黄圃腊味非遗文化周活动现场')
    ...
    add_image_placeholder(doc, '图3 中山美食旅游线路宣传海报')
...
def create_essay():
    ...
    add_body_paragraph(doc, '中山火炬高技术产业开发区作为国家级高新技术产业开发区，近年来在经济社会发展方面取得了显著成就。本文通过对火炬开发区2020年至2025年期间的产业发展、城市建设、民生改善、生态环境等方面的变化进行系统梳理和分析，总结其发展经验，探讨面临的挑战，并提出相应的对策建议。研究表明，火炬开发区坚持创新驱动发展战略，积极融入粤港澳大湾区建设，在产业转型升级、城市品质提升、民生福祉改善等方面实现了跨越式发展，为中山市高质量发展提供了有力支撑。')
    ...
    add_body_paragraph(doc, '近五年来，面对复杂多变的国内外经济形势和新冠疫情的冲击，火炬开发区坚持稳中求进工作总基调，完整准确全面贯彻新发展理念，积极融入粤港澳大湾区建设，统筹推进疫情防控和经济社会发展，取得了令人瞩目的发展成就。本文旨在对火炬开发区近五年的突出变化进行系统研究，总结发展经验，为未来发展提供参考。')
    
    # 二、产业发展方面的突出变化
    add_section_title(doc, '二、产业发展方面的突出变化')
    add_body_paragraph(doc, '（一）产业结构持续优化升级。近五年来，火炬开发区坚持"产业强区"战略，大力推动产业结构调整和转型升级。一方面，加快传统优势产业改造提升，推动家电、五金、纺织等传统产业向智能化、绿色化方向发展；另一方面，积极培育发展战略性新兴产业，形成了以健康医药、智能装备、新一代信息技术、新能源等为主导的现代产业体系。据统计，2024年火炬开发区高新技术企业数量达到350家，较2020年增长超过60%，高新技术产业产值占规模以上工业总产值的比重超过70%。')
    ...
    add_image_placeholder(doc, '图1 中山生命科学园园区实景')
    ...
    add_body_paragraph(doc, '（四）创新能力大幅提升。火炬开发区高度重视科技创新，近五年研发投入强度持续保持在较高水平。区内建有省级以上研发机构超过50家，其中国家级研发机构5家。2024年，火炬开发区专利申请量和授权量分别较2020年增长85%和92%，创新活力持续迸发。')
    ...
    add_image_placeholder(doc, '图2 火炬开发区城市新貌')
    ...
    add_body_paragraph(doc, '（一）居民收入水平稳步提高。近五年来，火炬开发区居民人均可支配收入保持较快增长，2024年达到6.5万元，较2020年增长约35%。城乡居民收入差距持续缩小，共同富裕取得新进展。就业形势保持稳定，城镇登记失业率控制在3%以内。')
    ...
    add_image_placeholder(doc, '图3 火炬开发区生态公园景观')
    ...
    run = p.add_run('[1] 中山火炬高技术产业开发区管委会. 火炬开发区"十四五"发展规划[Z]. 2021.')
    set_font(run, size=10)
    
    p = doc.add_paragraph()
    pf = p.paragraph_format
    pf.line_spacing = Pt(20)
    run = p.add_run('[2] 中山市统计局. 中山统计年鉴2024[M]. 北京: 中国统计出版社, 2024.')
    set_font(run, size=10)
    
    p = doc.add_paragraph()
    pf = p.paragraph_format
    pf.line_spacing = Pt(20)
    run = p.add_run('[3] 广东省人民政府. 粤港澳大湾区发展规划纲要实施情况报告[R]. 2023.')
    set_font(run, size=10)
    
    p = doc.add_paragraph()
    pf = p.paragraph_format
    pf.line_spacing = Pt(20)
    run = p.add_run('[4] 中山火炬高技术产业开发区统计局. 火炬开发区2024年国民经济和社会发展统计公报[R]. 2025.')
    set_font(run, size=10)
    
    p = doc.add_paragraph()
    pf = p.paragraph_format
    pf.line_spacing = Pt(20)
    run = p.add_run('[5] 张三, 李四. 国家级高新区高质量发展路径研究[J]. 区域经济评论, 2023(3): 45-52.')
    set_font(run, size=10)
