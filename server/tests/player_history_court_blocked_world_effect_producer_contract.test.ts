...
async function runCourtBlockedWorldEffectProducerContract() {
  rmSync(join(process.cwd(), 'tmp', 'player-history-court-blocked-world-effect'), { recursive: true, force: true })

  const port = await getAvailablePort()
  const baseUrl = `http://127.0.0.1:${port}`
  const tail: TailState = { stdout: [], stderr: [] }
  const child = spawnBackend(port, tail, {
    SESSION_STATE_PERSIST_PATH: buildSessionPersistPath('player_history_court_blocked_world_effect_session_state'),
  })
  ...
  try {
    ...
    const blockedResolutions = readArray(courtSession.resolutions)
      .map((item) => readObject(item))
      .filter((item) => item.decision !== 'passed')
    assert.ok(blockedResolutions.length > 0, 'advanceTick should include at least one blocked or held Court resolution')
    ...
    const blockedEvent = eventItems.find((item) => item.action === 'court_world_effect_blocked')
    assert.ok(blockedEvent, 'runtime events should include a Court blocked/no-change world-effect receipt')
    assert.equal(blockedEvent.success, false)
    const metadata = readObject(readObject(blockedEvent).metadata)
    assert.equal(metadata.playerHistoryCategory, 'court')
    assert.equal(metadata.playerHistoryTitle, '决议未生效')
    assert.equal(metadata.playerHistoryActorName, '洛阳朝议')
    assert.equal(metadata.playerHistoryResultLabel, '未生效')
    ...
    assert.equal(metadata.courtWorldEffectState, 'world_effect_blocked')
    assert.ok(String(metadata.courtNoChangeReason).length > 0)
    ...
    assert.equal(
      anonymousCards.some((card) => card.category === 'court' && card.title === '决议未生效'),
      false,
      'anonymous player history must not expose private Court blocked/no-change receipts',
    )

    const join = await requestJson(baseUrl, '/api/session/join', 'POST', {
      factionId: 'player',
      playerName: 'court_blocked_owner',
    })
    ...
    const courtCard = cards.find((card) => card.category === 'court' && card.title === '决议未生效')
    assert.ok(courtCard, 'owner player history should expose Court blocked/no-change receipt as a court card')
    assert.equal(courtCard.actorName, '洛阳朝议')
    assert.equal(courtCard.resultLabel, '未生效')
    assert.equal(courtCard.nextActionLabel, '查看朝议')
    assert.equal(courtCard.sharePolicy, undefined, 'private Court blocked receipt should not become shareable')
    assert.equal(courtCard.shareStateLabel, '暂不可分享')

    const visiblePayload = visibleCardText(courtCard)
    for (const required of ['决议未生效', '洛阳朝议', '未生效', '查看朝议']) {
      assert.ok(visiblePayload.includes(required), `Court blocked visible copy should include ${required}`)
    }
    for (const forbidden of [
      ...
      assert.equal(visiblePayload.includes(forbidden), false, `Court blocked visible copy leaked implementation term: ${forbidden}`)
    }
  } finally {
...
runCourtBlockedWorldEffectProducerContract().then(() => {
  console.log('[player_history_court_blocked_world_effect_producer_contract] all checks passed')
}).catch((error) => {
  console.error('[player_history_court_blocked_world_effect_producer_contract] failed:', error)
  process.exitCode = 1
})
